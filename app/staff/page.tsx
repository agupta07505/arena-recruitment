...
export default async function StaffPage() {
  ...
  const [{ data: applicationRows }, { data: assignmentRows }, { data: reviewRows }, { data: staffRows }, { data: campaign }, { data: auditRows }] = await Promise.all([
    supabase.from("applications").select("id, status, submitted_at, applicant:profiles(full_name, scholar_id, email, academic_year, branch, gender), position:positions(id, title, division, capacity)").neq("status", "draft").order("submitted_at", { ascending: false }),
  ...
  const applications: QueueApplication[] = (applicationRows ?? []).map((application) => {
    ...
    return { id: application.id, applicantName: applicant?.full_name ?? "", scholarId: applicant?.scholar_id ?? "", email: applicant?.email ?? "", year: applicant?.academic_year ?? null, branch: applicant?.branch ?? "", gender: applicant?.gender ?? null, positionId: position?.id ?? "", position: position?.title ?? "Unknown position", positionCapacity: position?.capacity ?? 0, division: position?.division ?? "operations", status: application.status, submittedAt: application.submitted_at, reviewCount: reviews.length, averageScore: averages.length ? averages.reduce((sum, value) => sum + value, 0) / averages.length : null };
  });
