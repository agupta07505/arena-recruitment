...
function CampaignControl({ campaign, readiness }: { campaign: StaffDashboardProps["campaign"]; readiness: StaffDashboardProps["readiness"] }) {
  ...
  return <form className={styles.campaignForm} onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const action = String((event.nativeEvent as SubmitEvent).submitter?.getAttribute("value") ?? "save") as "save" | "publish" | "close"; startTransition(async () => setMessage((await updateCampaignAction({ campaignId: campaign.id, opensAt: new Date(String(data.get("opensAt"))).toISOString(), closesAt: new Date(String(data.get("closesAt"))).toISOString(), action })).message)); }}><label><span>Opens</span><input defaultValue={campaign.opens_at ? new Date(campaign.opens_at).toISOString().slice(0,16) : ""} name="opensAt" required type="datetime-local" /></label><label><span>Closes</span><input defaultValue={campaign.closes_at ? new Date(campaign.closes_at).toISOString().slice(0,16) : ""} name="closesAt" required type="datetime-local" /></label><div><button disabled={pending} type="submit" value="save">Save draft</button>{campaign.status === "open" ? <button disabled={pending} type="submit" value="close">Close campaign</button> : <button disabled={pending || !readiness.turnstile || !readiness.email} type="submit" value="publish">Publish campaign</button>}</div>{message && <p>{message}</p>}</form>;
}
