...
import overrides from "./dashboard-overrides.module.css";
...
export function StaffDashboard({ applications, audits, campaign, readiness, roles, staffCount }: StaffDashboardProps) {
  ...
  const degreeOptions = Array.from(new Set(applications.map((application) => application.degree).filter(Boolean)));
  const yearOptions = Array.from(new Set(applications.map((application) => application.year).filter(Boolean)));
  const visible = useMemo(() => applications.filter((application) => {
    ...
    return haystack.includes(query.toLowerCase()) && (status === "all" || application.status === status) && (position === "all" || application.position === position) && (degree === "all" || application.degree === degree) && (year === "all" || application.year === year);
  }), [applications, degree, position, query, status, year]);
  ...
  const positionDemand = positionOptions.map((name) => ({ name, count: applications.filter((application) => application.position === name).length }));
  ...
  return <>
    ...
    <section className={styles.queueSection}>
      ...
      <div className={`${styles.filters} ${overrides.filters}`}>
        ...
        <label><span>Degree</span><select onChange={(event) => setDegree(event.target.value)} value={degree}><option value="all">All degrees</option>{degreeOptions.map((value) => <option key={value}>{value}</option>)}</select></label>
        <label><span>Year</span><select onChange={(event) => setYear(event.target.value)} value={year}><option value="all">All years</option>{yearOptions.map((value) => <option key={value}>{value}</option>)}</select></label>
        <div className={styles.resultCount}><strong>{visible.length.toString().padStart(2, "0")}</strong><span>results</span></div>
      </div>
      {(roles.includes("admin") || roles.includes("observer")) && <a className={styles.exportLink} href={`/api/staff/export?q=${encodeURIComponent(query)}&status=${encodeURIComponent(status)}&position=${encodeURIComponent(position === "all" ? "all" : applications.find((item) => item.position === position)?.positionId ?? "all")}&degree=${encodeURIComponent(degree)}&year=${encodeURIComponent(year)}`}>Download organised Excel workbook ↗</a>}
      <div className={styles.queueTable} role="table">
        ...
        {visible.map((application) => <article key={application.id} role="row">
          <div><strong>{application.applicantName || "Unnamed applicant"}</strong><small>{application.scholarId || application.email} · {application.degree} · Year {application.year || "—"}</small></div>
    ...
    <section className={styles.analyticsSection}><div className={styles.sectionLead}><div><span>02 / Internal intelligence</span><h2>Recruitment<br /><em>signal.</em></h2></div><p>Use the filters above to view candidates by applied position, degree, and academic year.</p></div><div className={styles.analyticsGrid}><article><span>Unique applicants</span><strong>{uniqueApplicants}</strong><small>{applications.length} total applications</small></article><article><span>Female applicants</span><strong>{new Set(applications.filter((item) => item.gender === "Female").map((item) => item.email)).size}</strong><small>unique candidates</small></article><article><span>Male applicants</span><strong>{new Set(applications.filter((item) => item.gender === "Male").map((item) => item.email)).size}</strong><small>unique candidates</small></article><article><span>Reviews complete</span><strong>{reviewed}</strong><small>applications with reviewer signal</small></article></div><div className={styles.chartGrid}><div><h3>Pipeline / exact state</h3>{funnel.map((item) => <div className={styles.barRow} key={item.stage}><span>{formatApplicationStatus(item.stage)}</span><i style={{ width: `${applications.length ? Math.max(2, item.count / applications.length * 100) : 2}%` }} /><b>{item.count}</b></div>)}</div><div><h3>Applications / by position</h3>{positionDemand.length ? positionDemand.map((item) => <div className={styles.barRow} key={item.name}><span>{item.name}</span><i style={{ width: `${applications.length ? Math.max(2, item.count / applications.length * 100) : 2}%` }} /><b>{item.count}</b></div>) : <p>No applications yet.</p>}</div></div></section>
